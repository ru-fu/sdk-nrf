...
When creating an application that requires child images (for example, a bootloader), you can use Partition Manager to control where in memory each image should be placed.
...
It is not enabled in single-image builds.
...
Placeholder partitions
   A placeholder partition does not contain an image, but is reserved for future use.
...
Phony partitions
   A phony partition does not reserve space, but is used to get a reference to a flash area.
   For example, a sub-partition is a phony partition.

The start addresses and sizes for image partitions are used in the preprocessing of the linker script for each image.
For placeholder partitions, the values are not used as input to any linker script.
...
This file must be stored in the same folder as the :file:`CMakeLists.txt` file of the sub-image.
...
inside: list
   ...
   ``partition_name`` is added to the ``span`` list of the first existing phony partition in the list.
...
The following example shows what kind of preprocessing is possible:
...
For each image, Partition Manager's cmake code infers the following paths from the name and from other global properties:
...
The partition manager script outputs a single :file:`partitions.yml` file.
...
Generated output
================
After the main partition manager script has run, another script runs which takes the :file:`parititons.yml` as input, and creates a number of output files:

   * A C header file :file:`pm_config.h` for each sub-image and the root application.
   * A Kconfig file :file:`pm.config`.

The C header file is used in the C code, while the Kconfig file is imported in CMake.
Both these files contain, among other things, the start address and size of all partitions.

Usage
=====
When using Partition Manager in your application, you should be aware of the functionality it adds.
...
CMake
-----
The CMake variables from Partition Manager are typically used through generator expressions, because these variables are made available at the end of the CMake configure stage.
...
For example, if a device includes a non-upgradable first-stage bootloader and an upgradeable application, the application image to be upgraded must be linked to the same address as the one that is deployed.
...
.. note::
  Sub-images with the build strategy *partition_name*\ _BUILD_STRATEGY_SKIP_BUILD or
  *partition_name*\ _BUILD_STRATEGY_USE_HEX_FILE must define a static partition to ensure correct placement of the dynamic partitions.
