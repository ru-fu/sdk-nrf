...
class NcsInclude(SphinxDirective):
    ...
    def run(self):
        ...
        if docset is None:
            raise self.severe('Need to supply a "docset" option')

        if docset not in self.config.ncs_include_mapping:
        ...
        if dedent or auto_dedent:
            ...
            if dedent:
                if dedent > min_spaces:
                    raise self.severe('Cannot dedent the amount of spaces '
