...
.. TODO: do we have a bit more information? At least some link?
...
You can download it from the `nRF9160 product website (compatible downloads)`_.
...
.. TODO: where can the user get these patches?
...
#. Create a binary file that contains the new image.
      ...
      To create a binary file for a bootloader upgrade, make sure that (:option:`CONFIG_SECURE_BOOT`) is enabled and build MCUboot as usual.
      The build will create a binary file that contains two variants of the upgradable bootloader, one for each bootloader slot.
      See :ref:`upgradable_bootloader` for more information.

#. Make the binary file available for download.
     Upload the file to a web server that is compatible with the :ref:`lib_download_client` library.
     One way of doing this is to upload the file to an Amazon Web Services Simple Storage Service (AWS S3) bucket.
     See the :ref:`lib_aws_fota` documentation for instructions.

     Your application must be able to retrieve the URL for the binary file.
     This URL can be hardcoded in the application, or you can use functionality like AWS jobs to provide the URL dynamically.

The full FOTA procedure depends on where the binary file is hosted for download.
